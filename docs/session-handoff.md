...
- while carrying a bag on foot, the player uses the dedicated `player-carry-*` sprite set
...
- standing on a free bag no longer draws an extra pickup indicator over the player
...
- Curtis is rendered with a bold red outline during `spot`, `alert`, and `police`
...
Win presentation flow:

- the player freezes into a 2-second celebration animation
- the win cutscene slides down over the frozen gameplay scene
- win stats reveal after the overlay settles
- the win screen currently reports completion time and aggregate Curtis alert seconds
- there is still no separate score formula beyond those reported stats
...
- the win cutscene now renders as a sliding overlay screen rather than replacing the gameplay canvas
